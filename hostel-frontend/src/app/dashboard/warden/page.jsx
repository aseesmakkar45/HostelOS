...
export default function WardenDashboard() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      try {
        ...
        const insightsRes = await axios.get('/warden/insights').catch(() => null);
        const logsRes = await axios.get('/gate/logbook').catch(() => null);
  ...
  const handleAction = (type, id) => {
    if (showToast) showToast(`Action ${type} triggered on ID ${id}`, 'info');
  };
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-6 border-l border-slate-200 pl-8">
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold text-slate-800">{insights?.congestionIndex || '92% Max'}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Residents</span>
                <span className="text-sm font-bold text-slate-800">{stats?.totalStudents || '—'} Active</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Pending</span>
                <span className="text-sm font-bold text-rose-500">{stats?.pendingComplaints || 0} Issues</span>
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white rounded-[2rem] border border-slate-100 p-8 card-hover relative overflow-hidden">
                  <div className="flex justify-between items-start relative z-10">
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">All Hostel Blocks</p>
                      </div>
                      <div className="flex items-end gap-2">
                        <p className="text-4xl font-extrabold text-slate-900">{stats?.totalVacantBeds ?? '—'}</p>
                    ...
                    <div className="text-right">
                      <div className="w-24 h-24 bg-indigo-600 rounded-full flex flex-col items-center justify-center text-white border-8 border-indigo-50">
                        <span className="text-xl font-bold">{stats?.occupancyRate ? `${stats.occupancyRate}%` : '—'}</span>
                  ...
                  <div className="mt-6 space-y-2 relative z-10">
                    <div className="flex justify-between text-xs font-bold">
                      ...
                      <span className="text-indigo-600">{stats?.totalStudents ?? '—'} Residents</span>
                    </div>
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 p-8 card-hover">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-extrabold text-slate-800">AI Capacity Monitor</h3>
                    <span className="px-3 py-1 bg-rose-100 text-rose-600 text-[10px] font-bold uppercase rounded-full flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-rose-500 animate-pulse" />
                      Live AI
                    </span>
                  </div>
                  {insights?.smartAlerts && insights.smartAlerts.length > 0 ? (
                    <div className="flex items-center gap-6 p-4 bg-indigo-50 border border-indigo-100 rounded-2xl">
                      <div className="w-16 h-16 bg-white rounded-xl flex items-center justify-center text-indigo-600 shrink-0 shadow-sm">
                        <Users className="w-8 h-8 text-indigo-500" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-bold text-slate-900">{insights.smartAlerts[0].title}</p>
                        <p className="text-xs text-indigo-700 font-semibold leading-relaxed mt-1">{insights.smartAlerts[0].details}</p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-6 p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                      <div className="w-16 h-16 bg-white rounded-xl flex items-center justify-center text-slate-400 shrink-0 shadow-sm">
                        <Users className="w-8 h-8 text-slate-400" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-bold text-slate-850">Operations Flow Normal</p>
                        <p className="text-xs text-slate-500 font-semibold leading-relaxed mt-1">AI monitors capacity thresholds. No congestions detected.</p>
                      </div>
                    </div>
                  )}
                  <div className="mt-4 grid grid-cols-2 gap-4">
                    <button 
                      onClick={() => showToast && showToast('Alert dismissed', 'success')}
                      className="py-2.5 bg-white border border-slate-200 text-slate-600 text-[10px] font-bold rounded-lg hover:bg-slate-50 transition-all uppercase tracking-widest cursor-pointer"
                    >
                      Dismiss
                    </button>
                    <button 
                      onClick={() => showToast && showToast('Gate staff deployed to Dining Hall', 'success')}
                      className="py-2.5 bg-indigo-600 text-white text-[10px] font-bold rounded-lg hover:bg-indigo-700 transition-all uppercase tracking-widest cursor-pointer"
                    >
                      Deploy Staff
                    </button>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {logs.slice(0, 5).map((log) => (
                        <tr key={log.id} className="group">
                          <td className="py-5 px-2">
                            <div className="flex items-center gap-3">
                              <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${log.person_name}`} className="w-9 h-9 rounded-lg bg-indigo-50" alt="avatar" />
                              <div className="min-w-0">
                                <p className="text-sm font-bold text-slate-800 truncate">{log.person_name}</p>
                                <p className="text-[10px] text-slate-400">Roll: {log.roll_number || 'N/A'}</p>
                              </div>
                            </div>
                          </td>
                          <td className="py-5 px-2 font-bold text-slate-600 text-sm">{log.room_number || 'Unallocated'}</td>
                          <td className="py-5 px-2">
                            <span className={`px-3 py-1 text-[10px] font-bold rounded-full uppercase ${
                              log.action?.toLowerCase() === 'exit' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'
                            }`}>
                              {log.action?.toLowerCase() === 'exit' ? 'Gate Check-Out' : 'Gate Check-In'}
                            </span>
                          </td>
                          <td className="py-5 px-2 text-xs font-medium text-slate-500">
                            {new Date(log.entry_time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                          </td>
                      ...
                      {logs.length === 0 && (
                        <tr>
                          <td colSpan="5" className="py-5 text-center text-xs text-slate-400 font-bold">No gate activity logged yet.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            <section className="col-span-12 lg:col-span-4 space-y-8">
              ...
              <div className="bg-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden card-hover">
                <div className="relative z-10 space-y-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-4xl font-extrabold">₹{(stats?.totalRevenue ?? 0).toLocaleString('en-IN')}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                      <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Paid</p>
                      <p className="text-lg font-bold">₹{(stats?.revenuePaid ?? 0).toLocaleString('en-IN')}</p>
                    </div>
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                      <p className="text-[10px] font-bold text-rose-400 uppercase tracking-widest">Pending</p>
                      <p className="text-lg font-bold">₹{(stats?.revenuePending ?? 0).toLocaleString('en-IN')}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => showToast && showToast('Generating revenue report...', 'info')}
              ...
              <div className="bg-indigo-50 rounded-[2rem] p-8 border border-indigo-100 relative overflow-hidden group">
                <div className="relative z-10 flex flex-col items-center text-center">
                  ...
                  <button 
                    onClick={() => showToast && showToast('Starting smart allocation matching...', 'info')}
