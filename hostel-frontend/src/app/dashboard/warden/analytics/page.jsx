...
export default function AIAnalytics() {
  ...
  useEffect(() => {
    async function fetchInsights() {
      try {
        const res = await axios.get('/warden/insights');
        if (res.data.success) {
  ...
  const diningForecastData = analyticsData?.diningForecastData || [
  ...
  const complaintDistributionData = analyticsData?.complaintDistributionData || [
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-40 gap-4">
              ...
              <p className="text-sm text-slate-500 font-bold">Compiling dynamic database telemetry...</p>
            </div>
          ) : (
              ...
              <div className="col-span-12 lg:col-span-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm card-hover">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="px-2 py-0.5 bg-rose-100 text-rose-600 text-[8px] font-bold uppercase rounded-full">Active</span>
                    </div>
                    <div className="flex items-end justify-between mt-2">
                      <h3 className="text-2xl font-black text-slate-900">{analyticsData?.congestionIndex || '100%'}</h3>
                      <span className="text-xs font-bold text-rose-500">Dining Load</span>
                    </div>
                  ...
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm card-hover">
                    ...
                    <div className="flex items-end justify-between mt-2">
                      <h3 className="text-2xl font-black text-slate-900">{analyticsData?.averageMttr || '4.2 Hrs'}</h3>
                  ...
                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm card-hover">
                    ...
                    <div className="flex items-end justify-between mt-2">
                      <h3 className="text-2xl font-black text-slate-900">{analyticsData?.validScansScore || '99.8%'}</h3>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm">
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-semibold mt-1">AI-driven predictive occupancy load calculated from active registrations and leaves.</p>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-semibold mb-6">Live categories analysis dynamically compiled from student filings.</p>
              ...
              <aside className="col-span-12 lg:col-span-4 space-y-8">
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm">
                  ...
                  <div className="space-y-5">
                    ...
                    {(analyticsData?.safetySweeps || []).map((sweep, index) => (
                      <div key={index} className={`flex gap-4 p-4 border rounded-2xl ${
                        sweep.title.includes('Violation') || sweep.title.includes('Tailgating')
                          ? 'border-rose-50 bg-rose-50/20 text-rose-700'
                        ...
                        <div className="flex-1">
                          <p className="text-xs font-bold text-slate-800">{sweep.title}</p>
                          <p className="text-[10px] text-slate-500 mt-1 leading-relaxed">{sweep.details}</p>
                          <p className="text-[8px] font-bold uppercase mt-1 text-slate-400">{sweep.time}</p>
                        </div>
                      </div>
                    ))}
